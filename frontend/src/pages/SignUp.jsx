...
const SignUp = () => {
    ...
    const registerUser = async () => {
        try {
            setLoading(true);
            const response = await axios.post('https://backendaitu.onrender.com/api/v1/user/signup', {
                username,
                firstName,
                lastName,
                password
        ...
        <div className="flex relative h-screen justify-center w-full items-center bg-gray-200 ">
            <div className="border bg-white flex flex-col rounded-lg lg:w-1/3 md:w-2/3 w-11/12">
                ...
                <div className="flex flex-col gap-2 md:px-12 px-6 md:py-6 py-4 border">
                    ...
                    <InputBox onChange={(e) => setUsername(e.target.value)} type={"email"} placeholder={"khana65@gmail.com"} label={"Email"} className="w-full border rounded border-slate-200" />
